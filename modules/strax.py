...
class Module(ModuleManager.BaseModule):
    ...
    def strax(self, event):
        ...
        """
        suggestion_greeting = ["I suggest", "We should attack immediately with"]

        command_greeting = ["We should attack now with",
                            "We must attack now with",
                            "I recommend attacking with",
        ...
        method_of_attack_a = ["full-frontal", "pincer", "surprise",
                              ...
                              "violent", "devastating", "superior", "fast-paced",
                              "fleet-wide", "stealth", "diversionary",
                              "exceptional", "point-blank", "night time"]
        ...
        attack_adjective = ["laser", "berserker", "acid", "armoured attack",
                            "proton", "three kinds of", "atomic", "toxic",
                            "explosive", "red-hot", "thermal",
                            "automated fire", "cluster", "enhanced germ",
                            "energy-drink-fueled", "battle ready", "Sontaran",
                            "military"]

        attack_object = ["bees", "chainsaws", "marmots", "acid", "monkeys",
                         "mines", "bombs", "snakes", "spiders", "knives",
                         "rockets", "sharks", "owls", "repurposed cybermats",
                         "cannons", "alligators", "ants", "gorillas",
                         "genetically enhanced cyber-elephants", "mechanoids",
                         "KGB agents", "MI5 operatives", "nuclear missiles"]
        ...
        method_of_attack = " an " + \
                           random.choice(method_of_attack_an) \
                            if random.choice([1,2]) == 1 \
                            else " a " + random.choice(method_of_attack_a)

        greeting_choice = random.choice([1, 2])

        greeting = random.choice(suggestion_greeting) \
            if greeting_choice == 1 else random.choice(command_greeting)

        exclamation = "?" if greeting_choice == 1 else "!"

        suggestion = greeting + method_of_attack + " " + \
                     random.choice(type_of_attack) + " with " + \
                     random.choice(attack_adjective) + " " + \
                     random.choice(attack_object) + " and " + \
                     random.choice(attack_object_two) + exclamation
